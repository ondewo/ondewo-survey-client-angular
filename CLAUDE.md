...
- **`generate` must not use `docker run -it`** (see the TTY point above); keep `-it` only on interactive `--entrypoint /bin/bash` debug commands.
...
```text
Release ONDEWO Survey Angular Client ${ONDEWO_SURVEY_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO Survey Angular Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO Survey Angular Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/survey-client-angular`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/survey-client-angular versions --json
```
